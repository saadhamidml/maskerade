...
class ExactGPModel(ExactGP):
    ...
    def sample_hyperparameters(self, n_samples: int = 1) -> torch.Tensor:
...
class GPModelCollection:
    ...
    def sample_hyperparameters(
            self, model_index: int, num_samples: int
    ) -> Sequence[Tensor]:
        model_components = self.create_model(model_index)
        model = model_components['model']
        theta = model.sample_hyperparameters(num_samples)
        del model
...
def build_model(
    ...
    if covariance_function['type'] == 'bayesian_gaussian_spectral_mixture':
        ...
        data_lh = GaussianLikelihood(SmoothedBoxPrior(1e-8, 1e-1))
        if dimensions > 1:
