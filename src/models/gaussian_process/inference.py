...
def bayesian_quadrature(
    ...
    covariances_chol = torch.cholesky(covariances)
    precisions_x_means = torch.cholesky_solve(
        means.unsqueeze_(-1), covariances_chol
    ).view(-1, n_predictands)
    # annoying workaround non-batched cholesky_inverse
    covariances_chol_inverses = torch.inverse(covariances_chol)
    del covariances_chol
    precisions = covariances_chol_inverses.transpose(1, 2) @ covariances_chol_inverses
    del covariances_chol_inverses
    ...
    while cur_index < n_weights:
        try:
            ...
            ks, ls = (kls // n_hyperparams).unsqueeze(1), (kls % n_hyperparams).unsqueeze(1)
            del kls
            covariance_sums = (covariances[ks] + covariances[ls]).squeeze(1)
            # P(m_k;m_l, sigma_k + sigma_l) ^0.5
            with gpytorch.settings.fast_computations(log_prob=False):
                mvs = MultivariateNormal(
                    means[ks].squeeze(1).squeeze(-1), covariance_sums
                )
                log_prob_weights = mvs.log_prob(means[ls].squeeze(1).squeeze(-1)) / 2

            # combined means and variances
            precision_sums = (precisions[ks] + precisions[ls]).squeeze(1)
            precision_sums_chol = torch.cholesky(precision_sums)
            # factor form square-root of normal (in log-space)
            log_normalisation_correction = (
                    (0.75 * n_predictands) * torch.tensor(2.).log()
                    + (n_predictands / 4) * torch.tensor(math.pi).log()
                    - 0.25 * precision_sums.logdet()
            )
            del precision_sums

            combined_means = torch.cholesky_solve((precisions_x_means[ls] + precisions_x_means[ks]).transpose(1,2), precision_sums_chol)

            # annoying workaround unbatched cholesky_inverse
            precision_sums_chol_inverses = torch.inverse(precision_sums_chol)
            del precision_sums_chol
            combined_covariances = 2 * precision_sums_chol_inverses.transpose(1, 2) @ precision_sums_chol_inverses
            del precision_sums_chol_inverses

            combined_variances = combined_covariances.diagonal(dim1=1, dim2=2)

            log_weight_factors = log_prob_weights + log_normalisation_correction
            del log_prob_weights
            del log_normalisation_correction
            # qw will have zeros so cannot stay in log space
            weights = quadrature_weights[ls, ks] * log_weight_factors.view(-1, 1).exp()

            output_mean += (weights.unsqueeze(1) * combined_means).sum(0).squeeze()
            output_variance += (weights * (combined_variances + combined_means.pow(2).squeeze(-1))).sum(0)
