...
def sample_initial(model: GPModelCollection, initialisation: Mapping):
    hyperparameters = []
    likelihoods = []
    num_samples_per_model = int(
        initialisation['num_samples'] / model.n_models
    )
    for model_index in range(model.n_models):
        try:
            samples = model.sample_hyperparameters(
                model_index, num_samples_per_model
            )
            ...
            for _ in range(num_samples_per_model):
                samples = model.sample_hyperparameters(model_index, 1)
...
def sample(
    ...
    if load_id is None:
        ...
        surrogate_model.covar_module.base_kernel.lengthscale = (
            surrogate['numerics']['initialisation']['lengthscale']
        )
    else:
    ...
    for i in range(n_iterations):
        ...
        model_index, acquisition = acquire(
            model,
            integrand_model,
        ...
        if (
            ...
            optimise_surrogate(
                ...
                **surrogate['numerics'],
                tensorboard=tensorboard,
                global_step_offset=(
                    (i + 1) / surrogate['numerics']['optimise_every'] + 1
                )
            )
            surrogate_model.covar_module.raw_outputscale.requires_grad_(False)
            surrogate_model.covar_module.base_kernel.raw_lengthscale.requires_grad_(False)
