...
class InterDomainIntegrandModel(IntegrandModel):
    ...
    def integral_posterior(
        ...
        data_integral_block, sample_posterior_means = self._data_integral_covariance(
            k_inv_z=k_inv_z,
            data_sample_covariances=data_sample_covariances
        )
        integral_integral_block = self._integral_integral_covariance(
            samples=samples,
            data_sample_covariances=data_sample_covariances,
        ...
        try:
            ...
            warnings.warn('Adding jitter to Integral Covariance')
    ...
    def compute_prediction_weights(self):
        ...
        try:
            ...
            self.num_samples = int(self.num_samples / 2)
            posterior, samples, _, covariance_blocks = self.integral_posterior(
                return_covariance_blocks=True
            )
    ...
    def _triple_kernel_integral(
        ...
        return product.div_(self.num_samples**2)
        ...
        for i in range(n_blocks):
            for j in range(i, n_blocks):
                covariance[i, j] = self._triple_kernel_integral(
                    samples[i],
                    sample_posterior_means[i],
                    samples[j],
                    sample_posterior_means[j]
                )
                # covariance[i, j] = (
                #     k_invs_zs[i].transpose(0, 1)
                #     @ tpl
                #     @ k_invs_zs[j]
                # )
        covariance = (
